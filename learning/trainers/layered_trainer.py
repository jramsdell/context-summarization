...
class Layer(object):
    def __init__(self, data_handler: DataHandler, trainer_class: Type[BaseTrainer], model, loss_function, lr=0.01, weight_decay=0.05):
        self.trainer = trainer_class(data_handler=data_handler, model=model, loss_function=loss_function, lr=lr)

    def train(self):
        self.trainer.train(n=200, report=False, weight_decay=0.5)
